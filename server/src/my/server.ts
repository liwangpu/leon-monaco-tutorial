...
import {  CancellationToken, FileSystemWatcher } from './vscodeAdapter';
import { ChildProcess, exec } from 'child_process';
import { LaunchInfo, launchOmniSharp, LaunchResult, LaunchTarget,  LaunchTargetKind } from './launcher';
...
export class OmniSharpServer {
    ...
    constructor( private eventStream: EventStream, private optionProvider: OptionProvider, private extensionPath: string, public decompilationAuthorized: boolean) {
    ...
    private async _start(launchTarget: LaunchTarget, options: Options): Promise<void> {
        ...
        try {
            launchInfo = await this._omnisharpManager.GetOmniSharpLaunchInfo('1.37.12', options.path, serverUrl, latestVersionFileServerPath, installPath, this.extensionPath);
        }
        catch (error) {
    ...
    private updateProjectInfo = async () => {
        ...
        let info = await requestWorkspaceInformation(this);
    ...
    public autoStart(preferredPath: string): Thenable<void> {
        ...
        return this._findLaunchTargets().then(async launchTargets => {
...
export async function requestWorkspaceInformation(server: OmniSharpServer) {
    ...
    if (response.MsBuild && response.MsBuild.Projects) {
        ...
        for (const project of response.MsBuild.Projects) {
            project.IsWebProject = false;

            const isProjectBlazorWebAssemblyProject = false;
            const isProjectBlazorWebAssemblyHosted = false;
